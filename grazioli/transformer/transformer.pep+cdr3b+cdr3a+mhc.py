...
for i in tqdm(range(5)):
    try:
        ...
        # Load data
        train_path = os.path.join(DATA_BASE, "train", f"train-{i}.csv")
        test_path = os.path.join(DATA_BASE, "test", f"test-{i}.csv")
        ...
        # Sample a smaller subset to match ERGO2's approach
        sample_size = 1178  # Match ERGO2's sample size per epoch
        if len(df_train) > sample_size:
            df_train = df_train.sample(n=sample_size, random_state=42)
            print(f"Sampled {sample_size} training samples")
        ...
        model = build_transformer_model(
            max_pep_len=MAX_PEP_LEN,
            max_cdr3_len=MAX_CDR3_LEN,
            d_model=64,  # equivalent to embed_dim
            num_heads=4,
            dff=128,  # equivalent to ff_dim
            num_encoder_layers=2,  # equivalent to num_blocks
            dropout_rate=0.1
        )
        ...
        history = model.fit(
            x_train, 
            y_train, 
            epochs=5,  # Reduced to 5 epochs to match NetTCR2
            batch_size=160,  # Match ERGO2's batch size
