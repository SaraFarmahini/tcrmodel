""" Updated ERGO-II model.py to be compatible with latest PyTorch and PyTorch Lightning versions
"""
...
class ERGO2(AbstractTCRModel):
    ...
    def train(self, train_df: pd.DataFrame, epochs: int = 5) -> None:
        ...
        trainer = Trainer(
            ...
            deterministic=True,
            max_epochs=epochs,
            logger=False  # Optional: integrate TensorBoardLogger
        )
    ...
    def test(self, test_df: Union[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if isinstance(test_df, str):
            ...
            raise NotImplementedError('Input shall be either a path string or a pd.DataFrame.')
